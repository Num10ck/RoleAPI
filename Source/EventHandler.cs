namespace CustomRoles
{
	...
	public class EventHandler
	{
		...
		private void OnVerified(VerifiedEventArgs ev)
		{
			...
			{
				InvisibilityExtensions.MakeInvisibleForPlayer(scp999, ev.Player);
			}
		}
		...
		private void OnChangingSpectatedPlayer(ChangingSpectatedPlayerEventArgs ev)
		{
			...
			{
				InvisibilityExtensions.RemoveInvisibilityForPlayer(ev.NewTarget, ev.Player);
			}

			if (_scp999role!.Check(ev.OldTarget))
			{
				InvisibilityExtensions.MakeInvisibleForPlayer(ev.OldTarget, ev.Player);
			}
		}
	}
}